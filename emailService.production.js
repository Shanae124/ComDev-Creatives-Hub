 ...
 * Supports: Gmail, SendGrid, AWS SES, Mailgun, Custom SMTP, Ethereal (dev)
 */
...
const createTransporter = async () => {
  ...
  switch(service) {
    ...
    case 'sendgrid':
      // SendGrid API (uses different pattern)
      if (!process.env.SENDGRID_API_KEY) {
        throw new Error('SendGrid requires SENDGRID_API_KEY in .env');
      }
      sgMail.setApiKey(process.env.SENDGRID_API_KEY);
      console.log('[Email] ✓ SendGrid API configured');
      return 'sendgrid'; // Return identifier, not transporter
      
    case 'ses':
      // AWS SES
      const aws = require('aws-sdk');
      if (!process.env.AWS_REGION || !process.env.AWS_ACCESS_KEY_ID) {
        throw new Error('AWS SES requires AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY in .env');
      }
      aws.config.update({
    ...
    case 'mailgun':
      ...
      if (!process.env.MAILGUN_API_KEY || !process.env.MAILGUN_DOMAIN) {
        throw new Error('Mailgun requires MAILGUN_API_KEY and MAILGUN_DOMAIN in .env');
      }
      const mg = require('nodemailer-mailgun-transport');
      console.log(`[Email] ✓ Mailgun configured for ${process.env.MAILGUN_DOMAIN}`);
      return nodemailer.createTransporter(mg({
        auth: {
    ...
    case 'smtp':
      ...
      if (!process.env.SMTP_HOST) {
        throw new Error('Custom SMTP requires SMTP_HOST in .env');
      }
    ...
    default:
      ...
      console.log('[Email] ⚠ Using Ethereal test account - emails will NOT be delivered to real inboxes');
